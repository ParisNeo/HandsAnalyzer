...
while cap.isOpened():
    ...
    if ha.nb_hands>0:
        ...
        for i in range(ha.nb_hands):
            ...
            thumb_stat = get_alignment_coefficient(hand.get_landmarks_pos(hand.thumb_landmark_indices))<0.7
            index_stat = get_alignment_coefficient(hand.get_landmarks_pos(hand.index_landmark_indices))<0.5
            middle_stat = get_alignment_coefficient(hand.get_landmarks_pos(hand.middle_landmark_indices))<0.5
            ring_stat = get_alignment_coefficient(hand.get_landmarks_pos(hand.ring_landmark_indices))<0.5
            pinky_stat = get_alignment_coefficient(hand.get_landmarks_pos(hand.pinky_landmark_indices))<0.5

            nb = int(thumb_stat)+int(index_stat)+int(middle_stat)+int(ring_stat)+int(pinky_stat)
